            ...
            <div class="screen-full">
                <div class="text-gray-800"> <!-- everything has to be wrapped in a single div -->
                    <div class="prose-size mb-8 mx-auto">
                        <div class="text-4xl md:text-5xl lg:text-6xl font-bold text-red-main w-full mb-8">{{ article.title }}</div>
                        <div class="text-base md:text-lg lg:text-xl xl:text-2xl w-full text-gray-400 mb-8">
                            <span>
                                <NuxtLink :to="'/blog/author/' + article.author.id" class="font-semibold underline lg:no-underline text-gray-800 lg:text-gray-400 hover:text-gray-800">{{ article.author.firstname }} {{ article.author.lastname }}</NuxtLink>
                    ...
                    <div class="text-base md:text-lg lg:text-xl xl:text-2xl font-bold mb-8 flex flex-row">
                        <span v-for="category in article.categories" :key="category.id">
                            <NuxtLink :to="'/blog/category/' + category.id">
                                <span class="mr-2 mb-2 rounded-sm lg:opacity-60 hover:opacity-100 bg-gray-200 px-3 py-1">
                                    {{ category.name }}
                                </span>
                            </NuxtLink>
                        </span>
                    ...
                    <div class="text-base md:text-lg lg:text-xl xl:text-2xl flex flex-row text-gray-400">
                        Written by&nbsp;
                        <NuxtLink :to="'/blog/author/' + article.author.id" class="text-gray-800 lg:text-gray-400 underline lg:no-underline hover:text-gray-800">
